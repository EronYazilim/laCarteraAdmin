...
export class girisComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.girisFormu.reset()
		...
		if (this.store.getItem("laCartera_admin_e_kull_adi") != '') {
			this.girisFormu.patchValue({
				e_kull_adi	: this.store.getItem("laCartera_admin_e_kull_adi"),
				e_beni_hatirla	: true,
			})
		}
  }
    ...
    if (!this.girisFormu.invalid) {
      ...
      if (this.responseData[0].S == "T") {
        ...
        if (this.requestData.e_beni_hatirla == true) {
          this.store.setItem("laCartera_admin_e_kull_adi", this.requestData.e_kull_adi)
        } else {
